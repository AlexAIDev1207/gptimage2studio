...
6. **部署与技术方案** — 先选部署方式，再反推数据库选择 → **确定后立即做部署方案预验证**
7. **用户转化路径** — 限次实现方式、限次后引导策略、博客 CTA 风格
8. **数据分析需求** — 分析工具选型、关键事件、转化路径追踪

### 部署方案预验证（第 6 项确定后立即执行）

当用户选定部署方案后，提醒做一次空项目验证（参考 SOP Phase 1.6 的详细步骤）：

```
通过 AskUserQuestion 确认：
  - 是否已用选定技术栈（Next.js 版本 + 数据库驱动）在目标平台做过部署验证？
  - 如未验证，建议立即做一次（30 分钟）：创建最小项目 → 部署 → 验证 SSR + API + DB
  - 如验证失败，当场切换备选方案（默认后备：Vercel + Supabase PostgreSQL）

⚠️ 不验证直接进入 Phase 2 的风险：可能在 Phase 5 发现平台不兼容，浪费数小时。
```
